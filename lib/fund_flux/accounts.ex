defmodule FundFlux.Accounts do
  @moduledoc """
  ...
  alias FundFlux.Accounts.{User, UserToken, UserNotifier, MoneyTransfer}
  ...
  def receive_money(user_id, amount) do
    %MoneyTransfer{}
    |> MoneyTransfer.changeset(%{user_id: user_id, amount: amount})
    |> Repo.insert()
  end

  @doc """
  Returns the list of money_transfers.

  ## Examples

      iex> list_money_transfers()
      [%MoneyTransfer{}, ...]

  """
  def list_money_transfers do
    from(m in MoneyTransfer, select: m)
    |> Repo.all()
  end
  ...
  """
 def create_money_transfer(%{from_user_email: from_email, to_user_email: to_email, amount: amount_input}) do
  # Convert the amount to a Decimal
  amount_decimal = Decimal.new(amount_input)

  Repo.transaction(fn ->
    # Fetch the users
    from_user = Repo.get_by(User, email: from_email)
    to_user = Repo.get_by(User, email: to_email)

    # Check if both users exist and if the sender has sufficient balance
    if from_user && to_user && Decimal.compare(from_user.balance, amount_decimal) != :lt do
      # Deduct from sender
      updated_from_user =
        from_user
        |> User.changeset(%{balance: Decimal.sub(from_user.balance, amount_decimal)})
        |> Repo.update!()

      # Add to receiver
      updated_to_user =
        to_user
        |> User.changeset(%{balance: Decimal.add(to_user.balance, amount_decimal)})
        |> Repo.update!()

      # Create the money transfer record
      %MoneyTransfer{}
      |> MoneyTransfer.changeset(%{from_user_email: from_email, to_user_email: to_email, amount: amount_decimal})
      |> Repo.insert!()

      {:ok, {updated_from_user, updated_to_user}}
    else
